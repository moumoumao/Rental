...
function findUser(pageNo,pageSize){
	...
	$.ajax({
		url:'/Rental/comment!findByMixAndPage.action',
		data:{'pageBean.pageSize':pageSize,'pageBean.pageNo':pageNo,
			'mycomment.user.userId':'${rental_user.userId}','mycomment.room.roomTitle':$('#title').val(),
			'mycomment.comContent':$('#desc').val()},
		type:'POST',
		dataType:'JSON',
		success:function(data){
			...
			if(data.aaData==''){
				...
				$.each(data.aaData, function(i,item){    
					var obj =item.room;  
					str+="<tr><td>"+isNullStr(obj.roomId)+"</td>"+
					"<td class='showcontent'>"+isNullStr(item.comContent)+" </td>"+
					"<td>"+isNullStr(obj.roomTitle)+" </td>"+
					"<td>"+isNullStr(item.user.userName)+" </td>"+
					"<td>"+isNullStr(item.createDate)+" </td>"+"<td>"+
					"<a href='javascript:showRoom("+obj.roomId+")'><span class='glyphicon glyphicon-search green'></span></a>  &nbsp;  "+" </td></tr>";
				 });
